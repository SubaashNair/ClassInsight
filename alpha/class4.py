...
# Admin section to set number of students
st.subheader("Admin Section")
admin_input = st.text_input("Enter admin password to set number of students:", type="password")

if admin_input == admin_password:
    st.write("Admin access granted.")
    num_students = st.number_input("Set number of students:", min_value=1, step=1)
    st.session_state.num_students = num_students
    st.success("Number of students has been set.")
else:
    st.warning("Please enter the correct admin password to set the number of students.")

# Ensure num_students is set before proceeding
if 'num_students' in st.session_state:
    ...
    # Only show admin section for viewing data if the correct admin password is entered
    admin_view_input = st.text_input("Enter admin password to view data and visualizations:", type="password")

    if admin_view_input == admin_password:
        st.write("Admin access granted to view data.")
    ...
    st.warning("The number of students has not been set by the admin.")
